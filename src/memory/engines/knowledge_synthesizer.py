...
class KnowledgeSynthesizer:
    ...
    async def synthesize(
        ...
        try:
            ...
            synthesized_text = await self._synthesize_with_llm(
                query=query, documents=relevant_docs, conflicts=conflicts
            )
    ...
    async def _retrieve_with_metadata_filter(
        ...
        try:
            ...
            documents = await self.semantic_tier.search(
                query_text=query, limit=max_results, filter_by=filter_query
            )
    ...
    def _detect_conflicts(self, documents: list[KnowledgeDocument]) -> list[dict[str, Any]]:
        ...
        conflict_tag = self.domain_config.get("conflicts", {}).get(
            "conflict_tag", "CONFLICT_DETECTED"
        )
        ...
        recommendations = [doc for doc in documents if doc.knowledge_type == "recommendation"]
            ...
            if conflicts and self.domain_config.get("conflicts", {}).get("explain_conflicts", True):
                ...
                for conflict in conflicts:
                    if "doc_ids" in conflict:
                        ...
                        conflict_note += (
                            f"- Document {conflict['doc_id']} is marked as conflicting\n"
                        )
