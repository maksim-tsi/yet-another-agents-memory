...
class MemoryQueryInput(BaseModel):
    ...
    l2_weight: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Weight for L2 Working Memory (default: 0.3)"
    )
    l3_weight: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Weight for L3 Episodic Memory (default: 0.5)"
    )
    l4_weight: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Weight for L4 Semantic Memory (default: 0.2)"
    )
...
class GetContextBlockInput(BaseModel):
    ...
    max_turns: int = Field(
        default=20, ge=1, le=100, description="Maximum recent turns to include (default: 20)"
    )
    max_facts: int = Field(
        default=10, ge=1, le=50, description="Maximum facts to include (default: 10)"
    )
    format: str = Field(
        default="structured",
...
async def memory_query(
    ...
    weights = SearchWeights(l2_weight=l2_weight, l3_weight=l3_weight, l4_weight=l4_weight)

    try:
        # Execute hybrid search
        results = await memory_system.query_memory(
            session_id=session_id, query=query, limit=limit, weights=weights
        )
        ...
        for i, result in enumerate(results, 1):
            ...
            if tier == "L2" and "fact_type" in metadata:
                formatted_results.append(
                    f"   Type: {metadata['fact_type']}, CIAR: {metadata.get('ciar_score', 'N/A')}"
                )
            elif tier == "L3" and "topics" in metadata:
                topics = ", ".join(metadata["topics"][:3])
                formatted_results.append(
                    f"   Topics: {topics}, Facts: {metadata.get('fact_count', 0)}"
                )
            elif tier == "L4" and "knowledge_type" in metadata:
                formatted_results.append(
                    f"   Type: {metadata['knowledge_type']}, Confidence: {metadata.get('confidence_score', 'N/A')}"
                )
...
async def get_context_block(
    ...
    try:
        ...
        context = await memory_system.get_context_block(
            session_id=session_id, min_ciar=min_ciar, max_turns=max_turns, max_facts=max_facts
        )
        ...
        if format == "text":
            ...
            if context.significant_facts:
                ...
                for i, fact in enumerate(context.significant_facts[:5], 1):
                    fact_preview = (
                        fact.content[:100] + "..." if len(fact.content) > 100 else fact.content
                    )
                    summary.append(
                        f"{i}. [{fact.fact_type or 'N/A'}] {fact_preview} (CIAR: {fact.ciar_score:.2f})"
                    )
