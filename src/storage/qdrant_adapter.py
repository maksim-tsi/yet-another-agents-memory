...
class QdrantAdapter(StorageAdapter):
    ...
    async def retrieve(self, id: str) -> dict[str, Any] | None:
        ...
        async with OperationTimer(self.metrics, "retrieve"):
            ...
            try:
                # Retrieve point
                points = await self.client.retrieve(collection_name=self.collection_name, ids=[id])
    ...
    async def scroll(
        ...
        async with OperationTimer(self.metrics, "scroll"):
            ...
            try:
                ...
                logger.debug(
                    f"Scroll returned {len(formatted_results)} results from {target_collection}"
                )
                return formatted_results
    ...
    async def store_batch(self, items: list[dict[str, Any]]) -> list[str]:
        ...
        try:
            ...
            for item in items:
                ...
                point = PointStruct(id=point_id, vector=item["vector"], payload=payload)
                points.append(point)
    ...
    async def retrieve_batch(self, ids: list[str]) -> list[dict[str, Any] | None]:
        ...
        try:
            ...
            points = await self.client.retrieve(collection_name=self.collection_name, ids=ids)
    ...
    async def _get_backend_metrics(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            if isinstance(vectors_config, dict):
                ...
                first_config = next(iter(vectors_config.values())) if vectors_config else None
                vector_size = (
                    first_config.size if first_config and hasattr(first_config, "size") else 0
                )
            else:
                # Single vector case
                vector_size = (
                    vectors_config.size if vectors_config and hasattr(vectors_config, "size") else 0
                )
    ...
    async def create_collection(self, name: str, config: dict[str, Any] | None = None) -> bool:
        ...
        try:
            ...
            if config and "vectors" in config:
                ...
                vectors_config = VectorParams(
                    size=self.vector_size, distance=distance_map.get(self.distance, Distance.COSINE)
                )

            await self.client.create_collection(collection_name=name, vectors_config=vectors_config)
    ...
    async def update_collection(self, name: str, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Update collection
            await self.client.update_collection(collection_name=name, **config)
