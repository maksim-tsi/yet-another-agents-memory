...
class GroqChatSession(ChatSession):
    """OpenAI-compatible Groq chat session for baseline (no YAAM)."""

    is_local: bool = True
    model: str = field(default_factory=lambda: os.getenv("GROQ_MODEL", DEFAULT_GROQ_MODEL))
    base_url: str = field(default_factory=lambda: os.getenv("GROQ_API_BASE", DEFAULT_GROQ_BASE_URL))
    max_response_tokens: int = field(
        default_factory=lambda: int(os.getenv("GROQ_MAX_OUTPUT_TOKENS", "1024"))
    )
    temperature: float = field(default_factory=lambda: float(os.getenv("GROQ_TEMPERATURE", "0.0")))
    ...
    def _update_costs(self, completion: ChatCompletion) -> None:
        ...
        if not usage:
            return
        cost_in = float(os.getenv("GROQ_COST_IN", "0.0"))
        cost_out = float(os.getenv("GROQ_COST_OUT", "0.0"))
