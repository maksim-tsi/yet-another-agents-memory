...
@pytest.mark.skipif(asyncpg is None and psycopg is None, reason="asyncpg or psycopg not installed")
@pytest.mark.asyncio
async def test_postgres_connection(config):
    ...
    if asyncpg is not None:
        ...
        try:
            ...
            print(
                f"✓ PostgreSQL connected (asyncpg): {config['postgres_host']}:{config['postgres_port']}/{db_name}"
            )
        ...
        try:
            ...
            print(
                f"✓ PostgreSQL connected (psycopg): {config['postgres_host']}:{config['postgres_port']}/{db_name}"
            )
...
def test_neo4j_connection(config):
    ...
    try:
        ...
        driver = GraphDatabase.driver(
            uri, auth=(config["neo4j_user"], config["neo4j_password"]), connection_timeout=5
        )
...
def test_all_services_summary(config):
    ...
    try:
        r = redis.Redis(
            host=config["redis_host"], port=config["redis_port"], socket_connect_timeout=2
        )
