...
class TestMemoryLifecycleFlow:
    ...
    async def test_l1_to_l2_promotion_with_ciar_filtering(
        ...
        if provider_name == "groq":
            model_name = test_settings.llm_providers["fallback"][
                "model"
            ]  # openai/gpt-oss-120b (now fallback)
        elif provider_name == "google-pro":
            model_name = test_settings.llm_providers["fallback_reasoning"][
                "model"
        ...
        print(
            f"✅ Promoted {len(promoted_facts)} facts from {len(turns)} turns using {provider_name}"
        )
        print(
            f"📊 Latencies: L1 store={latencies['l1_store_ms']:.2f}ms, promotion={latencies['promotion_ms']:.2f}ms"
        )
    ...
    async def test_l2_to_l3_consolidation_with_episode_clustering(
        ...
        if hasattr(qdrant_adapter, "client") and qdrant_adapter.client:
            try:
                ...
                print(
                    f"DEBUG STEP2: Collection 'episodes' points_count={collection_info.points_count} (was {points_before} before)"
                )
        ...
        if all_results:
            for i, r in enumerate(all_results[:3]):
                print(
                    f"DEBUG: Result[{i}] session_id={r.get('session_id')}, score={r.get('score')}"
                )
        ...
        print(
            f"DEBUG: Scrolling Qdrant - session={test_session_id}, collection={l3_tier.collection_name}"
        )
        ...
        if qdrant_results:
            for i, r in enumerate(qdrant_results[:3]):
                print(
                    f"DEBUG: ScrollResult[{i}] session_id={r.get('session_id')}, episode_id={r.get('episode_id')}"
                )
        ...
        LIMIT 10
        """
        neo4j_results = await neo4j_adapter.execute_query(
            neo4j_query, {"session_id": test_session_id}
        )
        ...
        print(
            f"✅ Created episodes indexed in both Qdrant ({len(qdrant_episode_ids)}) and Neo4j ({len(neo4j_episode_ids)})"
        )
    ...
    async def test_l3_to_l4_distillation_with_knowledge_synthesis(
        ...
        stats = await distillation_engine.distill(session_id=test_session_id, track_provenance=True)
        ...
        print(
            f"✅ Distilled {len(knowledge_docs)} knowledge documents from {len(episodes)} episodes using {provider_name}"
        )
    ...
    async def test_full_lifecycle_end_to_end(
        ...
        promotion_engine = PromotionEngine(
            l1_tier, l2_tier, topic_segmenter, fact_extractor, ciar_scorer
        )
        ...
        start_distillation = time.perf_counter()
        distillation_stats = await distillation_engine.distill(
            test_session_id, track_provenance=True
        )
        latencies["distillation_ms"] = (time.perf_counter() - start_distillation) * 1000
        ...
        l4_search = await typesense_adapter.search(
            {"q": "*", "query_by": "content", "filter_by": f"session_id:={test_session_id}"}
        )
        ...
        print(
            f"✅ Full lifecycle complete: {len(turns)}→{len(l2_facts)}→{l3_episode_count}→{l4_doc_count}"
        )
        print(f"📊 Total latency: {sum(latencies.values()):.2f}ms")
        print(
            f"📊 Breakdown: L1={latencies['l1_store_ms']:.0f}ms, Promotion={latencies['promotion_ms']:.0f}ms, "
        )
        print(
            f"   Consolidation={latencies['consolidation_ms']:.0f}ms, Distillation={latencies['distillation_ms']:.0f}ms"
        )
...
class TestLifecycleStreams:
    ...
    async def test_promotion_event_triggers_consolidation(
        self, test_session_id, cleanup_redis_keys
    ):
...
class TestCrossNavigationQueries:
    ...
    @pytest.mark.asyncio
    async def test_unified_memory_query_across_all_tiers(self, test_session_id, full_cleanup):
    ...
    async def test_context_block_assembly_from_l1_and_l2(
        self, test_session_id, cleanup_redis_keys, cleanup_postgres_facts
    ):
...
class TestNetworkLatencyValidation:
    ...
    @pytest.mark.asyncio
    async def test_cross_node_query_latency_under_threshold(self, test_session_id):
    ...
    @pytest.mark.asyncio
    async def test_hybrid_query_latency_with_real_network(self, test_session_id, full_cleanup):
...
SUPPLY_CHAIN_CONVERSATION_TEMPLATES = [
    ...
    (
        "user",
        "I need to check on shipment MAEU{idx:07d}. It was supposed to arrive at the Port of Los Angeles yesterday but I haven't received any updates.",
    ),
    (
        "assistant",
        "I've located shipment MAEU{idx:07d}. It's currently delayed at customs due to incomplete documentation. The expected clearance time is 48-72 hours.",
    ),
    # User asks about safety stock
    (
        "user",
        "Our safety stock levels for SKU-{idx:05d} are critically low. We're down to 15 days of supply and lead time from the supplier is 30 days.",
    ),
    (
        "assistant",
        "I recommend placing an emergency order with our backup supplier in Vietnam. They can deliver within 14 days at a 12% premium. I've also flagged this for the procurement team.",
    ),
    # Carrier delay notification
    (
        "user",
        "We just received notification that carrier COSCO is experiencing port congestion in Shanghai. This affects our Q1 inventory replenishment.",
    ),
    (
        "assistant",
        "I've identified 12 affected shipments totaling $2.3M in inventory value. Alternative routing through Ningbo port adds 5 days transit time but avoids the congestion.",
    ),
    # Quality control issue
    (
        "user",
        "Quality inspection failed for batch BT-{idx:06d}. Found 8% defect rate, exceeding our 2% threshold.",
    ),
    (
        "assistant",
        "I've initiated a supplier corrective action request (SCAR) and placed a hold on remaining inventory from this batch. Root cause analysis meeting scheduled for tomorrow.",
    ),
    # Demand forecast update
    (
        "user",
        "Marketing just revised Q2 demand forecast upward by 35% for the electronics category due to new product launch.",
    ),
    (
        "assistant",
        "I've updated the demand planning model. This creates a 45,000 unit gap in our current procurement plan. I recommend activating our contract manufacturing agreement with Flex.",
    ),
    # Supplier financial risk
    (
        "user",
        "Our credit monitoring service flagged supplier ACME-{idx:04d} with deteriorating financial health. They supply 40% of our packaging materials.",
    ),
    (
        "assistant",
        "I've initiated dual-sourcing evaluation with three alternative suppliers. Estimated qualification time is 6-8 weeks. Recommend building 60-day buffer stock as mitigation.",
    ),
    # Transportation cost optimization
    (
        "user",
        "Freight costs have increased 23% this quarter. We need to optimize our transportation network to stay within budget.",
    ),
    (
        "assistant",
        "Analysis shows consolidating shipments from 3 distribution centers to 2 regional hubs would reduce costs by $180K annually. Trade-off is 0.5 day increase in average delivery time.",
    ),
    # Inventory write-off discussion
    (
        "user",
        "We have 5,000 units of product XYZ-{idx:05d} that will expire in 30 days. Current sell-through rate won't clear this inventory.",
    ),
    (
        "assistant",
        "Options: 1) Flash sale at 40% discount (projected recovery: $125K), 2) Donate for tax benefit ($45K), 3) Liquidator sale ($80K). Recommend option 1 given brand positioning.",
    ),
    # Customs compliance issue
    (
        "user",
        "Border control in Germany flagged shipment for missing CE certification on product batch CE-{idx:06d}.",
    ),
    (
        "assistant",
        "Our compliance team confirmed the certification exists but wasn't included in shipping documents. I've expedited document submission. Expected release in 24-48 hours with no penalties.",
    ),
    # Warehouse capacity alert
    (
        "user",
        "Warehouse WH-{idx:03d} in Chicago is at 95% capacity. We have 3 inbound shipments arriving this week totaling 2,400 pallets.",
    ),
    (
        "assistant",
        "I've arranged overflow storage at third-party facility 15 miles away. Rate is $0.85/pallet/day. Also initiated review of slow-moving inventory for clearance opportunities.",
    ),
]
