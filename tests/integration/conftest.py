...
def real_llm_client(test_settings: TestSettings) -> tuple[LLMClient | None, str]:
    ...
    if gemini_key:
        try:
            ...
            client.register_provider(
                gemini, ProviderConfig(name=primary["name"], priority=1, enabled=True)
            )
            ...
            if "fallback_reasoning" in test_settings.llm_providers:
                ...
                client.register_provider(
                    gemini_pro, ProviderConfig(name="google-pro", priority=3, enabled=True)
                )
                print(
                    f"✓ Registered {test_settings.llm_providers['fallback_reasoning']['name']} Pro provider with model {test_settings.llm_providers['fallback_reasoning']['model']}"
                )
    ...
    if groq_key:
        try:
            ...
            client.register_provider(
                groq, ProviderConfig(name=fallback["name"], priority=2, enabled=True)
            )
            provider_registered = True
    ...
    if not provider_registered:
        pytest.skip(
            "No LLM provider available (GOOGLE_API_KEY and GROQ_API_KEY both missing/failed)"
        )
...
class ReportCollector:
    ...
    def write_report(self):
        ...
        report = {"date": date_str, "total_tests": len(all_tests), "tests": all_tests}
...
async def redis_adapter(test_session_id: str) -> AsyncGenerator[RedisAdapter, None]:
    ...
    adapter = RedisAdapter(config={"url": os.getenv("REDIS_URL", "redis://192.168.107.172:6379")})
    await adapter.connect()
    yield adapter
    await adapter.disconnect()
...
async def cleanup_postgres_facts(postgres_adapter: PostgresAdapter, test_session_id: str):
    ...
    try:
        await postgres_adapter.execute(
            "DELETE FROM working_memory WHERE session_id = $1", test_session_id
        )
        await postgres_adapter.execute(
            "DELETE FROM active_context WHERE session_id = $1", test_session_id
        )
...
async def full_cleanup(
    ...
    try:
        ...
        print(
            f"✅ L3 cleanup: {cleanup_counts['l3_neo4j']} Neo4j nodes/edges for session {test_session_id}"
        )
    except Exception as e:
    ...
    try:
        ...
        print(
            f"✅ L2 cleanup: {cleanup_counts['l2_postgres_wm']} facts, {cleanup_counts['l2_postgres_ac']} turns for session {test_session_id}"
        )
    except Exception as e:
    ...
    try:
        ...
        print(
            f"✅ L1 cleanup: {cleanup_counts['l1_redis']} Redis keys for session {test_session_id}"
        )
    except Exception as e:
