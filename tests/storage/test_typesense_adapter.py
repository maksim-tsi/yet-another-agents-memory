...
class TestTypesenseAdapterUnit:
    ...
    async def test_connect_success(self, mock_httpx_client):
        ...
        with patch(
            "src.storage.typesense_adapter.httpx.AsyncClient", return_value=mock_httpx_client
        ):
            config = {
                "url": "http://localhost:8108",
                "api_key": "test_key",
    ...
    async def test_store_success(self, mock_httpx_client):
        ...
        with patch(
            ...
            data = {"content": "Test document", "metadata": {"key": "value"}}
    ...
    async def test_store_with_id(self, mock_httpx_client):
        ...
        with patch(
            ...
            data = {"id": "provided-id", "content": "Test document", "metadata": {"key": "value"}}
    ...
    async def test_store_http_error(self, mock_httpx_client):
        ...
        store_response.raise_for_status = Mock(
            side_effect=httpx.HTTPStatusError(
                "Bad Request", request=Mock(), response=store_response
            )
        )
        mock_httpx_client.post.return_value = store_response
    ...
    async def test_retrieve_not_found(self, mock_httpx_client):
        ...
        retrieve_response.raise_for_status = Mock(
            side_effect=httpx.HTTPStatusError(
                "Not Found", request=Mock(), response=retrieve_response
            )
        )
        mock_httpx_client.get.return_value = retrieve_response
    ...
    async def test_search_success(self, mock_httpx_client):
        ...
        search_response.json = Mock(
            return_value={
                "hits": [
                    {"document": {"id": "doc-1", "content": "First document"}, "highlights": []},
                    {"document": {"id": "doc-2", "content": "Second document"}, "highlights": []},
                ]
            }
        )
        ...
        with patch(
            ...
            query = {"q": "document", "query_by": "content", "limit": 5}
...
class TestTypesenseAdapterIntegration:
    ...
    async def typesense_config(self):
            ...
            "schema": {
                ...
                "fields": [
                    ...
                    {"name": "created_at", "type": "int64", "optional": True},
                    {
                        ...
                        "type": "object",
                        "optional": True,
                    },  # Object type requires enable_nested_fields
                ],
            },
        }
        ...
        try:
            async with httpx.AsyncClient(
                headers={"X-TYPESENSE-API-KEY": config["api_key"]}, timeout=10.0
            ) as client:
                await client.delete(f"{config['url']}/collections/{collection_name}")
        except Exception:
    ...
    async def test_full_workflow(self, typesense_config):
        ...
        try:
            ...
            # Search documents
            search_query = {"q": "integration", "query_by": "content", "limit": 5}
...
class TestTypesenseAdapterBatchOperations:
    ...
    async def test_store_batch_documents(self, mock_httpx_client):
        ...
        with patch("httpx.AsyncClient", return_value=mock_httpx_client):
            ...
            batch_data = [
                {"id": f"doc-{i}", "content": f"Test document {i}", "session_id": "test-session"}
                for i in range(5)
            ]
...
class TestTypesenseAdapterSchemaAndSearch:
    ...
    async def test_search_with_facet_by(self, mock_httpx_client):
        ...
        mock_search_response.json = AsyncMock(
            return_value={
                ...
                "facet_counts": [
                    {
                        "field_name": "category",
                        "counts": [{"value": "A", "count": 1}, {"value": "B", "count": 1}],
                    }
                ],
            }
        )
        ...
        with patch("httpx.AsyncClient", return_value=mock_httpx_client):
            ...
            query = {"q": "test", "query_by": "content", "facet_by": "category", "limit": 10}
    ...
    async def test_search_with_sort_by(self, mock_httpx_client):
        ...
        with patch("httpx.AsyncClient", return_value=mock_httpx_client):
            ...
            query = {"q": "test", "query_by": "content", "sort_by": "timestamp:desc", "limit": 10}
    ...
    async def test_search_empty_results(self, mock_httpx_client):
        ...
        with patch("httpx.AsyncClient", return_value=mock_httpx_client):
            ...
            query = {"q": "nonexistent", "query_by": "content", "limit": 10}
    ...
    async def test_store_with_auto_id(self, mock_httpx_client):
        ...
        with patch("httpx.AsyncClient", return_value=mock_httpx_client):
            ...
            data = {"content": "Test document without ID", "session_id": "test-session"}
