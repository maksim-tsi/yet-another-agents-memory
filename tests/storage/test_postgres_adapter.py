...
async def test_search_with_filters(postgres_adapter, session_id):
    ...
    ids = []
    for i in range(5):
        data = {"session_id": session_id, "turn_id": i, "content": f"Message {i}"}
        record_id = await postgres_adapter.store(data)
        ids.append(record_id)

    # Search all
    results = await postgres_adapter.search({"session_id": session_id, "limit": 10})
    assert len(results) == 5

    # Search with limit
    results = await postgres_adapter.search({"session_id": session_id, "limit": 2})
...
async def test_delete(postgres_adapter, session_id):
    """Test deletion"""
    # Store
    data = {"session_id": session_id, "turn_id": 1, "content": "To be deleted"}
    record_id = await postgres_adapter.store(data)
...
async def test_ttl_expiration(postgres_adapter, session_id):
    ...
    results = await postgres_adapter.search({"session_id": session_id, "include_expired": False})
    assert len(results) == 0

    # Search with expired
    results = await postgres_adapter.search({"session_id": session_id, "include_expired": True})
...
async def test_context_manager():
    ...
    config = {"url": url, "table": "active_context"}
...
async def test_working_memory_table():
    ...
    config = {"url": url, "table": "working_memory"}
...
async def test_connection_error_handling():
    ...
    config = {"url": "postgresql://invalid:invalid@localhost:5432/invalid_db"}
...
async def test_store_without_connection():
    ...
    with pytest.raises(StorageConnectionError, match="Not connected"):
        await adapter.store({"session_id": "test", "turn_id": 1, "content": "test"})
...
async def test_unknown_table_error():
    ...
    config = {"url": url, "table": "unknown_table"}
...
async def test_count_with_session_filter(postgres_adapter, session_id):
    ...
    for i in range(3):
        record_id = await postgres_adapter.store(
            {"session_id": session_id, "turn_id": i, "content": f"Message {i}"}
        )
        ids.append(record_id)
...
async def test_search_empty_results(postgres_adapter):
    ...
    results = await postgres_adapter.search({"session_id": "nonexistent-session-xyz", "limit": 10})
    assert results == []
...
async def test_store_batch_partial_success(postgres_adapter, session_id):
    ...
    batch = [
        {"session_id": session_id, "turn_id": 100, "content": "Valid message 1"},
        {"session_id": session_id, "turn_id": 101, "content": "Valid message 2"},
    ]
...
async def test_retrieve_batch_with_missing_ids(postgres_adapter, session_id):
    ...
    real_id = await postgres_adapter.store(
        {"session_id": session_id, "turn_id": 200, "content": "Real message"}
    )

    # Retrieve batch with mix of real and fake IDs (use high integer IDs)
    results = await postgres_adapter.retrieve_batch([real_id, "999999990", "999999991"])
...
async def test_delete_batch_with_missing_ids(postgres_adapter, session_id):
    ...
    # Store one real record
    real_id = await postgres_adapter.store(
        {"session_id": session_id, "turn_id": 300, "content": "Delete me"}
    )

    # Delete batch with mix of real and fake IDs (use high integer IDs)
    results = await postgres_adapter.delete_batch([real_id, "999999992", "999999993"])
