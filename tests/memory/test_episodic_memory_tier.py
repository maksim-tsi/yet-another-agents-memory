...
class TestEpisodicMemoryTierStore:
    ...
    @pytest.mark.asyncio
    async def test_store_episode_requires_embedding(self, episodic_tier, sample_episode):
        """Test that embedding is required."""
        with pytest.raises(ValueError, match="Embedding required"):
            await episodic_tier.store(
                {"episode": sample_episode, "embedding": None, "entities": [], "relationships": []}
            )
...
class TestEpisodicMemoryTierSearch:
    ...
    async def test_search_with_filters(self, episodic_tier, sample_embedding):
        ...
        await episodic_tier.search_similar(
            query_embedding=sample_embedding, limit=10, filters=filters
        )
...
class TestEpisodicMemoryTierGraphQueries:
    ...
    async def test_query_graph_custom_cypher(self, episodic_tier):
        ...
        # Setup
        episodic_tier.neo4j.execute_query = AsyncMock(
            return_value=[{"name": "Entity1", "count": 5}]
        )
        ...
        episodic_tier.neo4j.execute_query.assert_called_once_with(query, {"param": "value"})
    ...
    async def test_get_episode_entities(self, episodic_tier):
        ...
        episodic_tier.neo4j.execute_query = AsyncMock(
            return_value=[
                {
                    "entity": {"entityId": "entity_1", "name": "Project Alpha", "type": "project"},
                    "r": {
                        "confidence": 0.9,
                ...
                {
                    "entity": {"entityId": "entity_2", "name": "John Doe", "type": "person"},
                    "r": {
                        "confidence": 0.95,
                        "factValidFrom": "2025-01-01T00:00:00+00:00",
                        "factValidTo": None,
    ...
    async def test_query_temporal(self, episodic_tier):
        ...
        episodes = await episodic_tier.query_temporal(
            query_time=query_time, session_id="session_1", limit=10
        )
...
class TestEpisodicMemoryTierQuery:
    ...
    async def test_query_by_session(self, episodic_tier):
        ...
        episodes = await episodic_tier.query(filters={"session_id": "session_1"}, limit=10)
    ...
    async def test_query_by_importance(self, episodic_tier):
        ...
        episodes = await episodic_tier.query(filters={"min_importance": 0.8}, limit=10)
...
class TestEpisodicMemoryTierHealthCheck:
    ...
    async def test_health_check_both_healthy(self, episodic_tier):
        ...
        episodic_tier.qdrant.health_check = AsyncMock(return_value={"status": "healthy"})
        episodic_tier.neo4j.health_check = AsyncMock(return_value={"status": "healthy"})
        episodic_tier.neo4j.execute_query = AsyncMock(return_value=[{"count": 42}])
    ...
    async def test_health_check_degraded(self, episodic_tier):
        ...
        episodic_tier.qdrant.health_check = AsyncMock(return_value={"status": "healthy"})
        episodic_tier.neo4j.health_check = AsyncMock(return_value={"status": "unhealthy"})
        episodic_tier.neo4j.execute_query = AsyncMock(return_value=[{"count": 0}])
...
class TestEpisodicMemoryTierContextManager:
    ...
    @pytest.mark.asyncio
    async def test_context_manager_lifecycle(self, mock_qdrant_adapter, mock_neo4j_adapter):
        """Test that context manager properly initializes and cleans up."""
        tier = EpisodicMemoryTier(
            qdrant_adapter=mock_qdrant_adapter, neo4j_adapter=mock_neo4j_adapter
        )
