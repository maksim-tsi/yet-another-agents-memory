...
class TestConsumerInitialization:
    ...
    async def test_consumer_group_creation(
        self, redis_client, consumer_group, consumer_name, cleanup_stream
    ):
        """Test creating a new consumer group."""
        consumer = LifecycleStreamConsumer(
            redis_client=redis_client, consumer_group=consumer_group, consumer_name=consumer_name
        )
...
class TestEventPublishing:
    ...
    @pytest.mark.asyncio
    async def test_publish_and_consume_event(self, producer, consumer, session_id, cleanup_stream):
        ...
        event_id = await producer.publish(
            event_type="test", session_id=session_id, data={"message": "hello"}
        )
    ...
    @pytest.mark.asyncio
    async def test_multiple_event_types(self, producer, consumer, session_id, cleanup_stream):
...
class TestPendingMessageRecovery:
    ...
    async def test_pending_message_recovery(
        self, producer, redis_client, consumer_group, session_id, cleanup_stream
    ):
...
class TestHealthCheck:
    ...
    @pytest.mark.asyncio
    async def test_health_check_healthy(self, consumer, cleanup_stream):
    ...
    @pytest.mark.asyncio
    async def test_health_check_with_handlers(self, consumer, cleanup_stream):
...
class TestMAXLENTrimming:
    ...
    @pytest.mark.asyncio
    async def test_stream_trimming(self, producer, redis_client, cleanup_stream):
        """Test that stream is trimmed at MAXLEN threshold."""
        stream_key = NamespaceManager.lifecycle_stream()

        # Publish many events
        for i in range(100):
            await producer.publish(event_type="test", session_id=f"session-{i}", data={"index": i})
...
class TestConcurrentConsumers:
    ...
    async def test_50_concurrent_consumers(
        self, producer, redis_client, session_id, cleanup_stream
    ):
        ...
        consumer_objects = await asyncio.gather(*[create_consumer(i) for i in range(num_consumers)])
