...
def mock_llm_provider():
    ...
    provider.generate = AsyncMock(
        return_value="""Based on the provided knowledge documents, here's the synthesized response:
...
@pytest.mark.asyncio
async def test_successful_synthesis(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
    """Test successful knowledge synthesis."""
    mock_semantic_tier.search.return_value = sample_knowledge_docs[:3]

    synthesizer = KnowledgeSynthesizer(
        semantic_tier=mock_semantic_tier, llm_provider=mock_llm_provider
    )
...
@pytest.mark.asyncio
async def test_metadata_filtering(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
    """Test metadata-first filtering."""
    # Only return docs matching the filter
    filtered_docs = [
        doc for doc in sample_knowledge_docs if doc.metadata.get("container_type") == "40RF"
    ]
...
@pytest.mark.asyncio
async def test_cache_hit(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
...
async def test_empty_results(mock_semantic_tier, mock_llm_provider):
    ...
    result = await synthesizer.synthesize(
        query="Unknown query with no matches", metadata_filters={"port_code": "ZZZZZ"}
    )
...
async def test_similarity_threshold_filtering(
    mock_semantic_tier, mock_llm_provider, sample_knowledge_docs
):
    ...
    result = await synthesizer.synthesize(query="Test query", max_results=5)
...
@pytest.mark.asyncio
async def test_conflict_detection(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
    ...
    result = await synthesizer.synthesize(
        query="How to handle refrigerated containers?", metadata_filters={"port_code": "USLAX"}
    )
...
@pytest.mark.asyncio
async def test_llm_failure_fallback(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
    ...
    result = await synthesizer.synthesize(
        query="Test query", metadata_filters={"port_code": "USLAX"}
    )
...
@pytest.mark.asyncio
async def test_cache_key_generation(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
    ...
    # First query
    result1 = await synthesizer.synthesize(query="Query 1", metadata_filters={"port_code": "USLAX"})
    key1 = result1["cache_key"]

    # Second query (different)
    result2 = await synthesizer.synthesize(query="Query 2", metadata_filters={"port_code": "USLAX"})
    ...
    result3 = await synthesizer.synthesize(query="Query 1", metadata_filters={"port_code": "USNYC"})
...
@pytest.mark.asyncio
async def test_cache_clear(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
...
@pytest.mark.asyncio
async def test_cache_expiration(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
...
@pytest.mark.asyncio
async def test_max_results_limit(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
...
@pytest.mark.asyncio
async def test_cache_stats(mock_semantic_tier, mock_llm_provider, sample_knowledge_docs):
    """Test cache statistics reporting."""
    mock_semantic_tier.search.return_value = sample_knowledge_docs[:3]

    synthesizer = KnowledgeSynthesizer(
        semantic_tier=mock_semantic_tier, llm_provider=mock_llm_provider, cache_ttl_seconds=3600
    )
