...
class TestAtomicPromotion:
    ...
    async def test_atomic_promotion_basic(
        self, lua_manager, redis_client, session_id, cleanup_keys
    ):
        ...
        promotable = await lua_manager.execute_atomic_promotion(
            l1_key=l1_key, l2_index_key=l2_index_key, ciar_threshold=0.5, batch_size=10
        )
...
class TestWorkspaceUpdate:
    ...
    async def test_workspace_update_initial(
        ...
        new_version = await lua_manager.execute_workspace_update(
            workspace_key=workspace_key, expected_version=-1, new_data=data, update_type="replace"
        )
...
class TestSmartAppend:
    ...
    @pytest.mark.asyncio
    async def test_smart_append_basic(self, lua_manager, redis_client, session_id, cleanup_keys):
        ...
        length = await lua_manager.execute_smart_append(
            list_key=list_key, data=data, window_size=20, ttl_seconds=3600
        )
    ...
    async def test_smart_append_windowing(
        ...
        for i in range(10):
            await lua_manager.execute_smart_append(
                list_key=list_key, data={"turn": i}, window_size=window_size, ttl_seconds=3600
            )
    ...
    async def test_smart_append_ttl_refresh(
        ...
        await lua_manager.execute_smart_append(
            list_key=list_key, data={"turn": 1}, window_size=20, ttl_seconds=10
        )
        ...
        await lua_manager.execute_smart_append(
            list_key=list_key, data={"turn": 2}, window_size=20, ttl_seconds=10
        )
