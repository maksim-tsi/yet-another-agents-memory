...
def redis_config():
    ...
    return {"url": os.getenv("REDIS_URL"), "window_size": 10, "ttl_seconds": 3600}
...
async def test_bench_retrieve_single(redis_adapter, session_id):
    ...
    record_id = await redis_adapter.store(
        {"session_id": session_id, "turn_id": 1, "content": "Benchmark message"}
    )
...
async def test_measure_latencies(redis_adapter, session_id):
    ...
    for _ in range(iterations):
        start = time.perf_counter()
        await redis_adapter.search({"session_id": session_id, "limit": 10})
...
async def test_bench_delete_operations(redis_adapter, session_id):
    ...
    for i in range(iterations):
        ...
        await redis_adapter.store({"session_id": temp_session, "turn_id": 1, "content": "Test"})
