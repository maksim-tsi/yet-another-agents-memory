...
class StorageBenchmark:
    ...
    async def run_benchmark(
        ...
        for name, adapter in self.adapters.items():
            ...
            try:
                adapter_results = await self._run_adapter_benchmark(adapter, name, workload)
                ...
                results["adapters"][name] = {"error": str(e), "status": "failed"}
    ...
    async def _execute_operation(self, adapter: Any, op: WorkloadOperation) -> Any:
        """
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run storage adapter performance benchmarks")
    parser.add_argument(
        "--size",
        type=int,
        default=10000,
    ...
    config = {"workload_size": args.size, "workload_seed": args.seed, "adapters": args.adapters}
