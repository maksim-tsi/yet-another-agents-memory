...
2. **Create (if non-existent):** If the .venv directory does not exist, you *must* run Poetry install:  
   * Remote host: `cd /home/max/code/mas-memory-layer && poetry install --with test,dev > /tmp/copilot.out 2>&1; cat /tmp/copilot.out`  
   * Local host: `poetry install --with test,dev > /tmp/copilot.out 2>&1; cat /tmp/copilot.out`  
